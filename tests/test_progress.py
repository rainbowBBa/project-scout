...
1. **진행 표시가 조사를 망치지 않는다** — 래퍼가 원본 호출을 그대로 통과시킨다.
2. **사람이 답하는 동안 진행 줄이 질문을 밀어내지 않는다** (001/09-출력양식.md).
   보류한 줄은 버리지 않고 답변 뒤에 순서대로 전부 찍는다.
"""
...
async def test_progress_is_held_while_another_component_asks(capsys):
    """실제 토폴로지 — 프롬프트는 워커 스레드, 진행 줄은 이벤트 루프에서 온다.

    `sleep` 없이 `Event` 핸드셰이크만 쓴다.
    """
