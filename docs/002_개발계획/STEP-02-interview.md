...
- [x] `uv run scout`를 서브커맨드 없이 실행하면 "프로젝트 설명 입력: "으로
  대화형으로 설명을 받고, "[인터뷰] 단계를 시작합니다." 배너 뒤에서 대화가
  진행되다가 결과 요약이 나온 뒤에 "...종료합니다." 배너로 끝난다. 질문은
  "인터뷰 봇: "으로 표시되고, 설문지 톤이 아니라 대화체 문장이다
