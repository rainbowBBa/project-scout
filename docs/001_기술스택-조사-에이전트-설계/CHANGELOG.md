...
## v34 (2026-09-04) — winner_reason의 첫 문장을 후보 고유 근거로

v33의 표를 실제 데이터로 보니 세 행의 첫 줄이 전부 같았다.

```
프론트엔드 …    1인 개발자가 1~2일 내 데모 완성이라는 단기 데드라인 제약에서 Gradio의…
LangChain …    1인 개발자가 1~2주 내 데모 버전을 완성하는 프로젝트 맥락에서, @langchain/…
HTTP …         1인 개발자가 1~2주 내 데모 완성이라는 프로젝트 제약에서 requests의…
```

표는 첫 문장을 잘라서 보여주므로 **어느 행이 무슨 결정인지 알 수 없었다.**

원인은 프롬프트의 순서였다. `winner_reason`의 규칙이 "1. 프로젝트 맥락의 제약을
인용한다 / 2. 2위와의 점수 차이"였고, LLM은 1번으로 **문장을 시작**했다. 지시를
정확히 따른 결과다.

**첫 문장은 그 후보만의 것으로 시작하게** 바꿨다 — 고르게 만든 `fact_id` 값이나
판정문 표현. 제약은 근거를 잇는 절로만 등장한다. 실측 반례 3개와 좋은 예 2개를
프롬프트에 박았다.

Sonnet으로 재실행한 결과:

```
프론트엔드 …    gh.issue_close_rate=0.98로 Streamlit(0.81)보다 문제…
LangChain …    BaseChatModel 추상 클래스와 ImageContent 타입을 직접 제공해…
HTTP …         gh.issue_close_rate 0.94로 httpx(0.0) 대비 문제 해결이…
```

세 줄이 각각 이긴 근거와 상대를 지목한다.

### 프롬프트만으로는 못 막는다

파이프라인이 정상으로 돌기 때문에 조용히 되돌아간다. `audit_headlines`가 결정
지점들의 첫 20자를 대조해 겹치면 `gaps`에 남긴다 — 불변식 5·18과 같은 성격의
이중 안전망이다. `solo_pick`은 LLM이 쓴 문장이 아니라 고정 문구라 제외한다.
