...
항목의 정본 목록은 [08-설정](docs/001_기술스택-조사-에이전트-설계/08-설정.md)이고
붙여 쓸 파일은 `.env.example`이다 — **새 설정을 만들면 둘 다 고친다.**

새 설정을 만들 때 지킬 것 셋:

1. **값을 함수 인자로 받는다** (`wrap_web_search(..., budget=...)` 패턴). 노드가
   `Settings`를 읽어 넘기고 기본값은 모듈 상수로 둔다. 함수가 모듈 상수를 직접 읽으면
   개발자 로컬 `.env`가 테스트 결과를 바꾸고, `.env`는 커밋되지 않아 재현도 안 된다
2. **범위가 있으면 Pydantic 제약(`ge`/`le`)으로 박는다.** 조용히 무시하는 클램프는
   "설정한 값과 실제 동작이 다르다"는 가장 찾기 어려운 버그가 된다
3. **MCP 서버 설정이면 `SCOUT_NET_` 접두사를 쓴다.** `SERVER_ENV_KEYS`를 통과하지
   못한 이름은 자식 프로세스에 도달하지 않는다 (`test_egress`가 검사한다)
...
| `winner_reason`을 제약 인용으로 시작하기 | 결정 지점마다 첫 줄이 같아져 보고서 표가 정보를 잃는다. 후보 고유 근거로 시작한다 |
...
| 버전 없이 `osv_query`를 부르기 | 고쳐진 과거 취약점까지 세어 성숙한 패키지가 위험해진다 (`django@3.2.0` 63건). 버전을 못 찾으면 **안 묻는다** |
| `osv` 미조회를 "0건"으로 대접하기 | `risk`가 근거 없이 후해진다. 항목을 건너뛴다 (4-evaluate) |
| 조회 실패 시 예외 던지기 | 요소 하나가 전체를 죽인다. `gaps`에 기록한다 (불변식 11) |
| `github_repo_health`의 `per_page=1`을 설정으로 빼기 | 값이 아니라 **개수 세는 기법**이다. 마지막 페이지 번호가 곧 기여자 수라서, 바꾸면 `gh.contributors`가 조용히 틀린 값이 되고 그게 `maturity` 입력이다 (불변식 5) |
| MCP 서버 설정을 만들고 `SERVER_ENV_KEYS`를 잊기 | 값이 자식 프로세스에 도달하지 않고 조용히 기본값이 쓰인다. 증상이 "환경변수가 안 먹는다"로만 보인다 |
| 프롬프트 문장에 예산·상한 숫자를 굳히기 | 설정을 내렸는데 프롬프트가 "3회까지"라고 말하면 에이전트는 3회 있다고 믿고 2회째에 소진 응답을 받는다. `{web_search_budget}`처럼 주입한다 |
