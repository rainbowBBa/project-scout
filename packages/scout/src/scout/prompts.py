...
첫 문장은 **이 후보만의 것**으로 시작한다. 그 후보를 고르게 만든 fact_id 값이나
판정문의 표현을 첫 문장에 넣는다. 보고서 표는 **첫 문장만** 잘라서 보여주므로,
첫 문장이 후보마다 달라야 표에서 결정 지점이 구분된다.

프로젝트 맥락의 제약은 인용하되 **문장을 그것으로 시작하지 않는다.** 근거를 잇는
절로 붙인다. 2위와의 overall 점수 차이는 숫자로 쓴다 (예: overall 4 대 3).

제약 재진술로 시작하면 결정 지점 3개의 첫 문장이 전부 같아진다 — 실측 반례:

  나쁨: "1인 개발자가 1~2일 내 데모 완성이라는 단기 데드라인 제약에서 Gradio의…"
  나쁨: "1인 개발자가 1~2주 내 데모 버전을 완성하는 프로젝트 맥락에서 @langchain/core는…"
  나쁨: "1인 개발자가 1~2주 내 데모 완성이라는 프로젝트 제약에서 requests의…"
        → 세 줄이 같은 말로 시작한다. 표에서 어느 행이 무슨 결정인지 알 수 없다

  좋음: "gh.issue_close_rate 0.98로 Streamlit(0.81)보다 문제 해결이 빠르고,
        Spaces 즉시 배포가 1~2일 데드라인에 맞는다. overall 5 대 5, maturity로 앞선다."
  좋음: "BaseChatModel 추상 클래스와 ImageContent 타입을 직접 제공해 커스텀 래퍼의
        필수 기반이 된다. 직접 구현 대비 overall 5 대 3."

refined_brief 의 문장을 옮겨 적지 않는다. 제약은 **왜 그 근거가 결정적인지**를
설명할 때만 등장한다.
