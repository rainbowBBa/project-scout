...
# `>`로 닫히는 것만 잡으므로 "a < b" 같은 평문은 건드리지 않는다
_TAG_RE = re.compile(r"<[^<>]{1,120}>")


def _as_list(value: Any) -> Any:
    """리스트 자리에 온 문자열을 흡수한다 — 의사 XML 태그를 항목 구분자로 보고 쪼갠다.

    Bedrock tool-use가 JSON 배열 대신 `<item>…</item>` 꼴을 흘려보낸다. 실패로 두면
    `Verdict` 파싱이 깨져 그 후보가 탈락 처리되고 dossier도 판정도 남지 않는다 —
    형식만 맞추는 자리이고 판단을 바꾸지 않는다.
    """
...
class Interview(BaseModel):
    ...
    refined_brief: str
    assumptions: StrList
    # 리포트 헤더의 첫 두 행 (5-report.md). 기본값이 있어야 예전 실행의
    # interview_json을 그대로 다시 렌더링할 수 있다
    title: str = ""
...
class Component(BaseModel):
    ...
    # 2개 미만이면 design이 코드로 needs_comparison=false로 내린다 (불변식 18)
    alternatives: StrList = Field(default_factory=list)
    constraints: StrList = Field(default_factory=list)
    # necessity("필요한가")와 다른 축 — "지금 비교해서 골라야 하는가" (불변식 17)
...
class CandidateDraft(BaseModel):
    """에이전트가 뽑는 후보. dossier는 코드가 ToolMessage에서 채운다 (불변식 4·13)."""
...
class FinalDesign(BaseModel):
    ...
    `shape`·`data_flow`가 `Architecture`와 같은 이름인 것은 의도다 — 그래야 `report`가
    나란히 놓기만 해도 v1↔v2 대조표가 된다.
    """

    summary: str
    shape: str
    data_flow: str
    # 비면 "기본틀 유지"다 — 지우지 않는다 (불변식 12)
